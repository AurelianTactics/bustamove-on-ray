...
run_experiments({
    'bustamove-ppo-hyperband': {
        ...
        'config': {
            ...
            'sgd_stepsize': lambda spec: random.uniform(1e-3, 1e-6),#random.choice([2e-4, 3e-5, 1e-5, 3e-6]),#grid_search([5e-4, 1e-4, 5e-5, 1e-5]),
            'timesteps_per_batch': lambda spec: random.randint(8,257),#64,#grid_search([16,32,64,128]),#40000,
            ...
            'clip_param': lambda spec: random.uniform(0.1, 0.4),#0.2,#grid_search([0.1, 0.2, 0.3]),
            ...
            'entropy_coeff':lambda spec: random.choice([0.0,0.003,0.001]),#grid_search([0.0,0.05,0.1]),
